...
- *anchor positions*: 3×3 grid (center, N, NE, E, SE, S, SW, W, NW) plus custom offset
  - keyboard shortcuts: =Q= (center), =X= (SE), =Z= (SW), =A= (NW), =W= (NE)
- *diameter input*: in millimeters (converted to grid units using the drawing's mm/unit scale)
- *mode toggle*: single or rivet
  - *single*: add cutout to selected layer(s)
  - *rivet*: add cutouts to all layers with the point; top/bottom layers get scaled diameter
- *rivet scale*: percentage for top/bottom layer diameter (only visible in rivet mode)

| action              | effect                                              |
|---------------------+-----------------------------------------------------|
| click               | add cutout to active layer                          |
| shift + click       | add cutout to all layers with that point            |
| alt + click         | remove hovered cutout from active layer             |
| alt + shift + click | clear all cutouts at point from all layers          |
...
| cmd/ctrl + =V=   | paste at cursor — enters floating mode (select tool)                            |
| arrow keys       | lift selection into floating mode and nudge 1 grid unit; or move floating paste 1 grid unit |
| shift + arrows   | lift/move all layers (selection); or move floating paste 10 grid units               |
| alt + arrows     | lift and nudge 10 grid units                                                                |
| drag inside selection | lift and drag selected points (active layer only; hold shift = all layers); bakes on release |
| =Enter=          | place / bake floating paste                                                     |
| =G= (hold)       | display Center of Gravity for visible layers in selection                       |
