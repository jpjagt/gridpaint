 ...
 */
function getNeighbors(x: number, y: number, pointsSet: Set<string>): boolean[][] {
 ...
 */
function generateElement0Path(centerX: number, centerY: number, elementSize: number, rotation: number): string {
 ...
 */
function generateElement1Path(centerX: number, centerY: number, elementSize: number, magicNr: number, rotation: number): string {
 ...
 */
function generateElement2Path(centerX: number, centerY: number, elementSize: number, magicNr: number, rotation: number): string {
 ...
 */
function generateBlobElementPath(x: number, y: number, elementSize: number, neighbors: boolean[][], gridSize: number): string {
  const centerX = x * gridSize + gridSize / 2
  const centerY = y * gridSize + gridSize / 2
  const magicNr = 0.553 // Bezier control point magic number for circular approximation
  ...
  if (isCenter) {
    ...
    for (const quadrant of quadrants) {
      if (quadrant.hasNeighbors) {
        ...
        pathParts.push(generateElement0Path(centerX, centerY, elementSize, quadrant.rotation))
      } else {
        // Element1: curved corner
        pathParts.push(generateElement1Path(centerX, centerY, elementSize, magicNr, quadrant.rotation))
      }
    }
  } else {
    ...
    for (const bridge of bridges) {
      if (bridge.condition) {
        ...
        pathParts.push(generateElement2Path(centerX, centerY, elementSize, magicNr, bridge.rotation))
      }
    }
  }
 ...
 */
function componentToSVGPath(component: string[], gridSize: number, borderWidth: number = 0): string {
  ...
  for (const pointStr of pointsToRender) {
    ...
    if (isActive || hasActiveNeighbors) {
      const elementPath = generateBlobElementPath(x, y, elementSize, neighbors, gridSize)
      if (elementPath) {
 ...
 */
export function exportLayerAsSVG(layer: Layer, gridSize: number, borderWidth: number, name: string): void {
  ...
  const paths = components.map(component => 
    componentToSVGPath(component, gridSize, borderWidth)
 ...
 */
export function exportAllLayersAsSVG(layers: Layer[], gridSize: number, borderWidth: number, name: string): void {
  const visibleLayers = layers.filter(layer => layer.isVisible && layer.points.size > 0)
