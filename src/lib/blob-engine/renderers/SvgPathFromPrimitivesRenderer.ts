...
export interface SvgPathFromPrimitivesDebugInfo {
  ...
  if (debugMode) {
    ...
    for (const [key, count] of counts) {
      if (count % 2 === 0) {
        cancelled += count
        console.log(
          `[SvgPathFromPrimitives] CANCEL (×${count}): ${edgeToString(representatives.get(key)!)}`,
        )
      }
    }
    console.log(
      `[SvgPathFromPrimitives] dedup: ${edges.length} total → ${cancelled} cancelled → ${edges.length - cancelled} boundary`,
    )
  }
...
function stitchEdgesIntoPaths(edges: Edge[], debugMode: boolean): Edge[][] {
  ...
  for (let startIdx = 0; startIdx < edges.length; startIdx++) {
    ...
    if (debugMode) {
      console.log(
        `[SvgPathFromPrimitives] starting path from edge ${startIdx}: ${edgeToString(edges[startIdx])}`,
      )
    }
    ...
    while (safety++ < 100000) {
      ...
      if (path.length > 1 && ptEq(currentEnd, path[0].a)) {
        if (debugMode)
          console.log(
            `[SvgPathFromPrimitives]   → closed after ${path.length} edges`,
          )
        break
      }
      ...
      if (candidates) {
        for (const { idx, flip } of candidates) {
          ...
          if (debugMode) {
            console.log(`[SvgPathFromPrimitives]   + ${edgeToString(oriented)}`)
          }
          break
        }
      }
      ...
      if (!found) {
        if (debugMode) {
          console.log(
            `[SvgPathFromPrimitives]   → STUCK at ${ptToString(currentEnd)} (${path.length} edges, open path)`,
          )
        }
        break
      }
...
export class SvgPathFromPrimitivesRenderer extends Renderer {
  private _lastDebugInfo: SvgPathFromPrimitivesDebugInfo | null = null
  ...
  getLastDebugInfo(): SvgPathFromPrimitivesDebugInfo | null {
  ...
  renderLayer(
    ...
    if (dbg) {
      console.log(
        `\n[SvgPathFromPrimitives] ── renderLayer ──────────────────────────`,
      )
      console.log(
        `[SvgPathFromPrimitives] primitives (${geometry.primitives.length}):`,
      )
      for (const p of geometry.primitives) {
        const rq =
          p.renderQuadrant !== undefined ? ` renderQ=${p.renderQuadrant}` : ""
        console.log(
          `[SvgPathFromPrimitives]   ${p.type.padEnd(16)} q${p.quadrant}${rq}  @ (${p.center.x},${p.center.y})  curveType=${p.curveType}`,
        )
      }
    }
    ...
    for (const primitive of geometry.primitives) {
      ...
      if (dbg) {
        const relOffset = getRelOffset(primitive)
        console.log(
          `[SvgPathFromPrimitives] → ${primitive.type} q${primitive.quadrant} relOffset=${relOffset} @ (${primitive.center.x},${primitive.center.y}) → ${edges.length} edges:`,
        )
        for (const e of edges) {
          console.log(`[SvgPathFromPrimitives]     ${edgeToString(e)}`)
        }
      }
    }

    if (dbg) {
      console.log(`[SvgPathFromPrimitives] total raw edges: ${allEdges.length}`)
    }
    ...
    if (dbg) {
      console.log(
        `[SvgPathFromPrimitives] boundary edges (${boundaryEdges.length}):`,
      )
    ...
    if (dbg) {
      console.log(
        `[SvgPathFromPrimitives] stitched into ${stitchedPaths.length} path(s):`,
      )
      for (let i = 0; i < stitchedPaths.length; i++) {
        const p = stitchedPaths[i]
        const closed = p.length > 1 && ptEq(p[p.length - 1].b, p[0].a)
        console.log(
          `[SvgPathFromPrimitives]   path[${i}] ${p.length} edges, ${closed ? "CLOSED" : "OPEN"}`,
        )
      }
    }
    ...
    if (dbg) {
      console.log(`[SvgPathFromPrimitives] SVG paths (${svgPaths.length}):`)
      for (const p of svgPaths) {
        console.log(`[SvgPathFromPrimitives]   ${p}`)
      }
      if (cutoutPaths.length > 0) {
        console.log(
          `[SvgPathFromPrimitives] cutout paths (${cutoutPaths.length}):`,
        )
